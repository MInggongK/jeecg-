...
版本中加入了关于 Jack-chen11提出的问题，如果遇到非jeecg-boot的目录，检测就会失效，希望可以加个自定义路径

这个之前已经说过，IWannaGetAll-v1.2.1工具使用的全jeecg-boot路径方式，所以导致bug问题很明显，误报率也很高

v4.0版解决了这个问题，在用户默认检测使用jeecg-boot目录外，接口测试也加入了all检测

用户填入抓取的接口，也可以填入自定义接口，进行测试，填入格式:

在之前的版本中可以fsdfds'程序会自动去除'号再进行拼接

v4.0版需要填入格式：不包含/  不留空格  如sdfs目录
